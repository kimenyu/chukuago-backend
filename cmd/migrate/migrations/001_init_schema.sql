...
CREATE TABLE IF NOT EXISTS runner_profiles (
    ...
    bio              TEXT,
    kyc_status       TEXT NOT NULL DEFAULT 'unsubmitted' CHECK (kyc_status IN ('unsubmitted','pending','approved','rejected')),
    vehicle_type     TEXT, -- foot/bike/car/etc (keep freeform for now)
    vehicle_plate    TEXT,
...
CREATE TABLE IF NOT EXISTS errands (
    ...
    CHECK (status IN ('draft','posted','bidding','assigned','in_progress','delivered','completed','cancelled','disputed','expired')),
...
-- Offers (bids) and also used for instant accept pricing
...
-- ----------------------------
-- Escrow, Wallets, Ledger, External Transactions, Withdrawals
-- ----------------------------
...
-- Ledger is the source of truth; balance can be derived but kept for fast reads.
CREATE TABLE IF NOT EXISTS wallet_ledger (
    ...
    CHECK (ledger_type IN (
           'escrow_hold','escrow_release','errand_payout','platform_fee','refund','tip',
           'withdrawal_request','withdrawal_paid','adjustment'
                          )),
...
-- External payment transactions (M-Pesa etc) + idempotency
...
CREATE TABLE IF NOT EXISTS withdrawals (
    ...
    amount           NUMERIC(14,2) NOT NULL,
    currency         TEXT NOT NULL DEFAULT 'KES',
    destination      TEXT NOT NULL, -- mpesa_number / bank account masked
    status           TEXT NOT NULL DEFAULT 'pending'
...
CREATE TABLE IF NOT EXISTS delivery_proofs (
    ...
    otp_hash     TEXT,        -- store hash, not raw OTP
    notes        TEXT,
    created_at   TIMESTAMPTZ NOT NULL DEFAULT now()
    );
